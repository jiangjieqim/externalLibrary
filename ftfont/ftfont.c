...
/**
 *@param *iWidth,*iHeight 单字输出的像素宽高,字的宽高  
 */
int ft_parse(void* pt,unsigned char* outBuffer,int fontw,int fonth,int *iWidth,int *iHeight,int* iTop,char* str){
	...
	//get=========================================================
	{
		...
		//提取数据
		{
			...
			int bitmap_top			= slot->bitmap_top;		//相对横基准线偏移
...
ft_load(unsigned char* outBuffer,int fontw,int fonth,int *iWidth, int *iHeight,char* str){
	...
	//get=========================================================
	{
		...
		{
			printf("your str:[%s] FT_GLYPH_FORMAT_BITMAP (可选的列表中找不到宽高符合要求的文本像素)fw=%d fh=%d\n",str,fontw,fonth);//fon文件属于bitmap,不能render
			assert(0);
		}
